...
import numpy as np
from hummingbird_eval.models import FeatureExtractorBeta as FeatureExtractor
from hummingbird_eval.models import FeatureExtractorSimple
from hummingbird_eval.eval_metrics import PredsmIoU

from hummingbird_eval.transforms import (
    get_hbird_val_transforms,
    get_hbird_train_transforms,
    get_hbird_train_transforms_for_imgs,
)

from hummingbird_eval.image_transformations import CombTransforms
from hummingbird_eval.voc_data import VOCDataModule
from hummingbird_eval.ade20kdata import Ade20kDataModule
...
class HbirdEvaluation:
    def __init__(
        ...
        if self.memory_size is not None:
            self.num_sampled_features = self.memory_size // (
                dataset_size * self.augmentation_epoch
            )
            ## create memory of specific size
            self.feature_memory = torch.zeros(
                (self.memory_size, self.feature_extractor.d_model)
            )
